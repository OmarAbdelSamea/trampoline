 ...
 *
 * OS data structure generated from application fac
...
/* Task decode identifier */
#define decode_id 0
CONST(TaskType, AUTOMATIC) decode = decode_id;

/* Task encode identifier */
#define encode_id 1
CONST(TaskType, AUTOMATIC) encode = encode_id;

/*=============================================================================
 * Declaration of Alarm IDs
 */

/* Alarm encode_blink identifier */
#define encode_blink_id 0
CONST(AlarmType, AUTOMATIC) encode_blink = encode_blink_id;
 ...
 */
/*-----------------------------------------------------------------------------
 * Task decode descriptor
 */
#define APP_Task_decode_START_SEC_CODE

#include "tpl_memmap.h"
/*
 * Task decode function prototype
 */

FUNC(void, OS_APPL_CODE) decode_function(void);
 ...
 * Task decode stack
 */
#define APP_Task_decode_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) decode_stack_zone[300/sizeof(tpl_stack_word)];
#define APP_Task_decode_STOP_SEC_STACK
#include "tpl_memmap.h"

#define decode_STACK {decode_stack_zone, 300}
...
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_core_context, OS_VAR) decode_int_context;

#define decode_CONTEXT &decode_int_context
...
/*
 * Static descriptor of task decode
 */
CONST(tpl_proc_static, OS_CONST) decode_task_stat_desc = {
  /* context                  */  decode_CONTEXT,
  /* stack                    */  decode_STACK,
  /* entry point (function)   */  decode_function,
  /* internal ressource       */  NULL,
  /* task id                  */  decode_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task decode
 */
VAR(tpl_proc, OS_VAR) decode_task_desc = {
  /* resources                      */  NULL,
#if WITH_OSAPPLICATION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_OSAPPLICATION */
...
/*-----------------------------------------------------------------------------
 * Task encode descriptor
 */
#define APP_Task_encode_START_SEC_CODE

#include "tpl_memmap.h"
/*
 * Task encode function prototype
 */

FUNC(void, OS_APPL_CODE) encode_function(void);
 ...
 * Task encode stack
 */
#define APP_Task_encode_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) encode_stack_zone[300/sizeof(tpl_stack_word)];
#define APP_Task_encode_STOP_SEC_STACK
#include "tpl_memmap.h"

#define encode_STACK {encode_stack_zone, 300}
...
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_core_context, OS_VAR) encode_int_context;

#define encode_CONTEXT &encode_int_context
...
/*
 * Static descriptor of task encode
 */
CONST(tpl_proc_static, OS_CONST) encode_task_stat_desc = {
  /* context                  */  encode_CONTEXT,
  /* stack                    */  encode_STACK,
  /* entry point (function)   */  encode_function,
  /* internal ressource       */  NULL,
  /* task id                  */  encode_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task encode
 */
VAR(tpl_proc, OS_VAR) encode_task_desc = {
  /* resources                      */  NULL,
#if WITH_OSAPPLICATION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_OSAPPLICATION */
...
tpl_stat_proc_table[TASK_COUNT+ISR_COUNT+1] = {
  &decode_task_stat_desc,
  &encode_task_stat_desc,
 ...
 */
/*-----------------------------------------------------------------------------
 * Alarm encode_blink descriptor
 *
 * This alarm does the activation of task encode
 */
...
CONST(tpl_task_activation_action, OS_CONST) encode_blink_action = {
  {
...
CONST(tpl_alarm_static, OS_CONST) encode_blink_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_ID == YES)
    /* id of the alarm for tracing  */  , encode_blink_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&encode_blink_action
};
...
VAR(tpl_time_obj, OS_VAR) encode_blink_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&encode_blink_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  50,
    /* date                         */  250,
